...
const PackageCard = ({ content, name, title, img, content2, content2_5, content3, content4, price, bestDeal, index, applyHoverEffect, onMouseEnter, onMouseLeave }) => (
    <div className={`flex justify-between flex-col px-10 py-12 rounded-[20px] max-w-[370px] md:mr-10 sm:mr-5 mr-0 my-5 feedback-card ${applyHoverEffect ? 'feedback-card-hover' : ''}`}
                ...
                <div className="flex flex-row items-center py-[6px] rounded-[10px] mb-2">
                    {index === 1 && (
                        <p className={`${styles.paragraph} mt-6`}>
                            <span className="text-white bg-black-gradient p-5 rounded-lg">{bestDeal}</span>
                        </p>
                    )}
                </div>
            </div>
